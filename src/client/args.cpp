...
parse_mapping_list(std::string_view list) {
    ...
    while (!list.empty()) {
        ...
        if (!mapping) {
            return std::unexpected(
                Error{.errc = TunnelErrc::invalid_mapping,
...
parse_mapping_file(const std::filesystem::path &file_path) {
    ...
    if (!file) {
        return std::unexpected(
            Error{.errc = TunnelErrc::file_open_failed,
...
std::expected<ClientConfig, Error>
parse_mapping_configuration(int argc, const char **argv) {
    ...
    try {
        ...
        return std::unexpected(Error{.errc = TunnelErrc::invalid_arguments,
                                     .context = err.what()});
    }
    ...
    if (auto file_path = program.present("-f")) {
        ...
        if (!file_mappings) {
            return std::unexpected(
                std::move(file_mappings).error().with("parsing --file"));
        }
        config.mappings.insert(config.mappings.end(), file_mappings->begin(),
                               file_mappings->end());
    }
    ...
    if (auto list = program.present("-l")) {
        ...
        if (!list_mappings) {
            return std::unexpected(
                std::move(list_mappings).error().with("parsing --list"));
        }
        config.mappings.insert(config.mappings.end(), list_mappings->begin(),
                               list_mappings->end());
    }

    if (config.mappings.empty()) {
        return std::unexpected(Error{.errc = TunnelErrc::empty_mapping,
                                     .context = "no port mappings provided"});
    }
