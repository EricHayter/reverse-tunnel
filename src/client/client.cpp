...
Client::Client(const std::string &addr_string,
    ...
    if (res != 0 || server_info == nullptr) {
        throw std::runtime_error(
            "Call to getaddrinfo() failed (Likely bad hostname or service)");
    }

    sock_fd_m = socket(server_info->ai_family, server_info->ai_socktype,
    ...
    if (err) {
        throw std::runtime_error(
            Error{.context = "Connection attempt with server failed"}
                .with(err->context)
                .context);
    }
}
...
std::optional<Error> Client::send_mapping_message() {
    ...
    std::vector<std::byte> msg_buffer((1 + (2 * mappings_m.size())) *
                                      sizeof(PortNum));
    ...
    for (const auto &[from_port, to_port] : mappings_m) {
        ...
        std::memcpy(std::span(msg_buffer).subspan(offset).data(),
                    &from_port_network, sizeof(from_port_network));
        offset += sizeof(from_port);
        PortNum to_port_network = htons(to_port);
        std::memcpy(std::span(msg_buffer).subspan(offset).data(),
                    &to_port_network, sizeof(to_port_network));
    ...
    if (err_code) {
        return Error{.context =
                         std::format("Failed to send data to server (errno {})",
                                     *err_code)};
    }
