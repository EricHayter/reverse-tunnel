...
MessageBufferManager::MessageBufferManager() : buffers_m(NUM_BUFFERS) {
    for (std::size_t i = 0; i < NUM_BUFFERS; i++) {
        available_buffers_m.insert(BufferId(i));
    }
}

std::pair<MessageBufferManager::BufferId, std::span<std::byte>>
MessageBufferManager::acquire_buffer() {
    std::unique_lock<std::mutex> lock(mut_m);
    cond_m.wait(lock, [this]() { return !available_buffers_m.empty(); });
